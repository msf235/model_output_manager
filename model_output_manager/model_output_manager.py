...
def get_dirs_and_ids_for_run(run_params, table_path='output/param_table.csv', compare_exclude=[], maximize=None):
    ...
    table_path = Path(table_path)
    table_dir = table_path.parents[0]
    out = _get_updated_table(compare_exclude, run_params, table_path, None,
                             maximize, False)
    merge_ids = out[-1]
...
def _get_updated_table(compare_exclude, table_params, table_path,
                       column_labels=None, maximize=None,
    ...
    new_cols_notset = new_cols.copy()
    if prompt_for_user_input:
        while new_cols_notset != []:
            new_col_key = new_cols_notset.pop()
            new_param_value = input(
    f"""New parameter '{new_col_key}' detected. Please enter value for previous runs,
    or press any key to set previous values of the new parameter to 'na'.""")
    ...
    if unique_to_param_df != []:
        if prompt_for_user_input:
            print(
"""Warning: parameter table contains keys not specified in
parameter dictionary. Setting these values to 'na'.""")
